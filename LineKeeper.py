...
def main():
    ...
    while True:
        ...
        if choice in 'Aa':
            ...
            if (dirty and (get_string('Save before quit? y/n', 'yes/no', 'y') in YES)):
...
def delete_item(items, dirty):
    index = get_integer('Delete string number or 0 to cancel', 'number', maximum=len(items), allow_zero=True)
    if index != 0:
        del items[index-1]
        return True
    return dirty
...
def get_choice(items, dirty):
    while True:
        ...
        if choice not in valid_choices:
            print('Invalid choice. Please enter one of "{valid_choices}"'.format(locals()))
            input('Press Enter to continue...')
        else:
            return choice
...
def choose_file():
    ...
    if not enter_filename:
        print_list(files)
        index = get_integer("Specify file's number (or 0 to create a new one)", 'name', maximum=len(files), alow_zero=True)
        if index == 0:
            enter_filename = True
        else:
